...
`source/launcher/` holds what the Playdate launcher shows before the app
runs: `card.png` (350x155, card view), `icon.png` (32x32, list view) and
`launchImage.png` (400x240, the splash while the game loads -- and it must
have **no transparency**). All three are generated by `tools/launcher.ps1`,
which draws them with the app's own graphics stack and fonts, so re-run that
rather than editing the PNGs.

**`pdxinfo` must say `imagePath=launcher`.** There is no default location:
without that line `pdc` still compiles the art into the bundle and the
launcher then never looks at it, so the game shows the generic placeholder
card and everything appears fine from the build output. `launcher/` is a
community convention, not an SDK one -- every SDK example declares its own
directory (`SystemAssets`, `system/`, `card`).

Keep `pdxinfo` to bare `key=value` lines. It is copied verbatim into the
bundle and parsed by the OS; no SDK example has comments in it, so don't
invent them. **Bump `buildNumber` for every sideload upload** -- the docs
say it's what determines whether a newer version is available.

The scene is a bus in front of Cluj: Saint Michael's spire on the left, the
cathedral dome on the right, a skyline of lit windows, the bus in front. Two
things it took a few passes to get right, both worth keeping:

- The skyline's *shortest* building has to clear the bus roof, or the city is
  reduced to a fringe poking over the top and the picture stops reading as
  "a bus in front of a city".
- The road is a kerb line and dashes on white, not a filled black band. Black
  ground put a second heavy mass under an already-black bus and flattened the
  whole thing.
...
| `ListScene` | all four lists — All routes, All stops, and the two favorites screens; Right/Left add and remove favorites; remembers its selected row per list, and a `refresh` list rebuilds on entry so it is never stale |
...
- Every screen is a `NobleScene` subclass in `source/scenes/`, with
  ...
  and own no sprites.
- **Scenes are rebuilt on the way back, not resumed**, so anything worth
  keeping across a round trip is held in a module-level table keyed by what
  it belongs to, and restored in `init`. Every drill-down screen does this:
  `ListScene` per list key, `StopScene` per stop, `TimetableScene` per
  route+direction (day, hour row *and* column — B from a trip used to dump
  you on the first hour of today). Forgetting this is the single most likely
  way to make navigation feel broken, and it isn't visible from reading one
  scene. `RouteScene` additionally takes an optional `stopId` so opening it
  from Stop Detail starts the line at the stop you were standing at rather
  than the far end of the route.
...
│   ├── launcher/             generated by tools/launcher.ps1 -- don't hand-edit
...
└── tools/
    ...
    ├── launcher.ps1          redraws source/launcher/ (card, icon, splash)
    ├── launcher/main.lua     the generator launcher.ps1 builds
